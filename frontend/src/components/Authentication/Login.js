...
const Login = (props) => {
  ...
  const handleLogin = async function (e) {
    ...
    if (!email || (!password && !document.getElementById("otp")?.value)) {
      showtoast("Missing fields", "Please enter email and password or OTP.", "warning");
      return;
    }
  ...
  const handlesendotp = async (e) => {
    ...
    try {
      ...
      if (response.status !== 200) {
        ...
        const info = resdata.otp ? `OTP: ${resdata.otp}` : "OTP sent to your email";
        showtoast("otp sent", info, "success");
      }
    } catch (error) {
      console.log(error);
    }
            ...
            <form>
              <Stack spacing={4}>
                ...
                <Button
                  ...
                  onClick={handleLogin}
                  isLoading={loggingIn}
                  loadingText={forgotpasswordshow ? "Logging in..." : "Logging in..."}
                >

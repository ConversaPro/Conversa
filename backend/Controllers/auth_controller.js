...
const register = async (req, res) => {
  ...
  const emailRegex = new RegExp(`^${String(email).trim().replace(/[.*+?^${}()|[\\]\\]/g, "\\$&")}$`, "i");
  ...
  const us = await User.findOne({ email: String(email).trim().toLowerCase() });
    us._id = new ObjectId();
    us.name = "AI Chatbot";
  us.email = String(email).trim().toLowerCase() + "bot";
  ...
  const bot = await User.findOne({ email: String(email).trim().toLowerCase() + "bot" });
...
const login = async (req, res) => {
  ...
  const emailRegex = new RegExp(`^${loginEmail.replace(/[.*+?^${}()|[\\]\\]/g, "\\$&")}$`, "i");
  const user = await User.findOne({ email: emailRegex });
...
const sendotp = async (req, res) => {
  try {
    ...
    if (!process.env.EMAIL || !process.env.PASSWORD) {
      console.log("EMAIL/PASSWORD not configured; dev mode OTP:", otp);
      return res.status(200).json({ message: "OTP generated (dev mode)", otp });
    }
